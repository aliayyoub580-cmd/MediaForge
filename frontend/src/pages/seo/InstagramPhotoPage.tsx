...
const faqs = [
  { q: 'How to download Instagram photos in original quality?', a: 'Paste the post URL into MediaForge Pro and download high-resolution JPG image files.' },
  { q: 'Does it support carousel posts with multiple photos?', a: 'Yes, MediaForge Pro processes multi-image carousel posts.' },
];

export default function InstagramPhotoPage() {
  const title = 'Instagram Photo Downloader HD — Save Instagram Posts Online';
  const description = 'Download Instagram photos and carousel posts in full resolution HD quality. Free online Instagram photo downloader with no login required.';
  const path = '/instagram-photo-downloader';

  const breadcrumbs = [
    { name: 'Instagram Downloader', url: '/instagram-downloader' },
    { name: 'Instagram Photos', url: path },
  ];
        ...
        <div className="container-max">
          <div>
            <h2 className="font-display text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-teal-50 mb-4">
              Download Full-Resolution Instagram Photos & Carousels
            </h2>
            <p className="text-slate-600 dark:text-teal-100/80 leading-relaxed text-sm sm:text-base">
              Save high quality image files and multi-photo carousel posts from Instagram directly to your computer or smartphone.
            </p>
          </div>

          <div className="flex flex-wrap gap-3 text-xs font-semibold">
            <Link to="/instagram-downloader" className="px-3.5 py-2 rounded-xl bg-white dark:bg-dark-800 text-teal-600 dark:text-teal-300 border border-teal-500/30 hover:scale-105 transition-all">
